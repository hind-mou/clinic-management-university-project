				...
				<div class='don'>
					<div class="table">
						...
						if(mysqli_num_rows($resultat)>0){
							...
							<div class='cell' data-title='Location' id=".$row['id_pat']."-".$row['date_a'].">
							...
							echo"</div>";
					 $sql = "SELECT COUNT(id_a) AS total FROM analyse";
...
echo"<option value='".$r["id_pat"]."'>".$r["nom"]."-".$r["prenom"]." (".$r["cin"].")</option> 
         ";}
 echo"</select><br>";
 echo"<select name='analyse' required='true'> <option value='' >Choisi type (prix)...</option>";
			...
			$(document).ready(function(){
				...
				// pour la modification
				 $(document).on ("click", ".m", function () {
				 					$("#phrase").html("modifier analyse");

       document.querySelector(".ajouter").style.visibility="visible";
            $(".ajouterf").load("analyse/editana.php",{
 ...
 $("select").select2();
			 ...
			 // pour la suppression
			  $(document).on ("click", ".s", function () {
				...
				if (confirm("voulez-vous vraiment supprimer ce analyse?")) {
   				$(".don").load("analyse/deleteana.php",{
              id:this.parentNode.id
