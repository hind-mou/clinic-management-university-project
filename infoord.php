...
<div class="contenu">
	<h3>Plus d'informations sur l'ordonnance</h3>
	<?php 
	$p=$_GET['id'];
	$sql="SELECT patient.nom,patient.prenom,patient.cin,rendez_vous.date_r,rendez_vous.id_u FROM rendez_vous,consultation,patient,ordonnance WHERE consultation.id_r=rendez_vous.id_r AND rendez_vous.id_pat=patient.id_pat AND ordonnance.id_c=consultation.id_c AND ordonnance.id_o=$p;";
	...
	$sql="SELECT nom FROM user WHERE id_u=$id_u;";
	$resultat=mysqli_query($conn,$sql);
	while($r=mysqli_fetch_assoc($resultat)){
		$user=$r['nom'];
}
	$sql="SELECT medicament.libelle,ord_med.posologie FROM ord_med,medicament WHERE ord_med.id_o=$p AND ord_med.id_m=medicament.id_m GROUP BY id_o;";
	$resultat=mysqli_query($conn,$sql);
	...
	<table>
		<tr>
			...
			<tr>
			<td style="font-weight: bold;">Nom et prenom :</td>
			<td><?php echo $nom." ".$prenom; ?></td>
		</tr></td>
		<tr>
			<td style="font-weight: bold;">date consultation :</td>
		...
		<tr>
			<td style="font-weight: bold;">medicaments et posologie</td>
			<td>
				<select>
					<?php 
					while($r=mysqli_fetch_assoc($resultat)){
						echo "<option>".$r['libelle']." :".$r['posologie']."</option>"; 
