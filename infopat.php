...
<div class="contenu">
	...
	while($r=mysqli_fetch_assoc($resultat)){
		...
		$prenom=$r['prenom'];
		$sexe=$r['sexe'];
		$date_n=$r['date_naissance'];
		$date_c=$r['date_creation'];
		$tel=$r['tel'];
		$email=$r['email'];
		$adresse=$r['adresse'];
		$maladies=$r['maladies_chroniques'];
		$id_n=$r['id_n'];

	}
	if($sexe==1)
{$sexe="femme";}else{$sexe="homme";}

		$sql="SELECT libelle FROM nationalite WHERE id_n=$id_n;";
	$resultat=mysqli_query($conn,$sql);
	while($r=mysqli_fetch_assoc($resultat)){
		$nationalite=$r['libelle'];
}
$sql="SELECT date_r,id_c FROM rendez_vous,consultation WHERE rendez_vous.id_r=consultation.id_r AND rendez_vous.id_p=$p;";

	$resultatr=mysqli_query($conn,$sql);
	
		$sql="SELECT date_p FROM paiement,consultation,rendez_vous,patient WHERE patient.id_pat=rendez_vous.id_pat AND rendez_vous.id_r=consultation.id_r AND consultation.id_c=paiement.id_c AND patient.id_pat=$p;";
	$resultatp=mysqli_query($conn,$sql);
	

$sql="SELECT date_a FROM patient_analyse WHERE id_pat=$p;";
	$resultata=mysqli_query($conn,$sql);
	...
	<table>
		<tr>
			...
			<td style="font-weight: bold;">date création :</td>
			...
			<td style="font-weight: bold;">Maladies chroniques :</td>
			<td><?php echo $maladies; ?></td>
		</tr></td>
		</tr>
		<tr>
			<td style="font-weight: bold;">consultations :</td>
			<td><select>
				<?php 
				if(mysqli_num_rows($resultatr)>0){
	while($r=mysqli_fetch_assoc($resultatr)){
		echo "<option>".$r['date_r']."</option>";
}}else {
	echo "<option>pas de consultation</option>";
}
		...
		<tr>
			<td style="font-weight: bold;">Analyses :</td>
			<td><select>
				<?php 
				if(mysqli_num_rows($resultata)>0){
	while($r=mysqli_fetch_assoc($resultata)){
		echo "<option>".$r['date_a']."</option>";
}}else {
	echo "<option>pas d'analyse</option>";
}
		...
		<tr>
			<td style="font-weight: bold;">paiements :</td>
			<td><select>
				<?php 
				if(mysqli_num_rows($resultatp)>0){
	while($r=mysqli_fetch_assoc($resultatp)){
		echo "<option>".$r['date_p']."</option>";
}}else {
	echo "<option>pas de paiements</option>";
}
