					...
					<nav class="gn-menu-wrapper">
						<div class="gn-scroller">
							<ul class="gn-menu">
								...
								<li>
									<a class="gn-icon gn-icon-archive" href="medicament.php">médicaments</a>
...
for ($i=1; $i<=$total_pages; $i++) {  // print href pour tous les pages
            echo "<a href='ordonnance.php?page=".$i."'";
...
<table style="margin-left: 30px;">
	...
	<tr>
		<td><select style="margin-bottom:10px;" name="med1"><option value='' >Choisi...</option><?php while($r=mysqli_fetch_assoc($resultat)){
			echo " <option value='".$r['id_m']."' >". $r['libelle']."</option>";}?></select></td>
<td><input type="text" name="pos1"></td>
	</tr>
	<tr>

		<td><select style="margin-bottom:10px;"  name="med2"><option value='' >Choisi...</option><?php  $resultat=mysqli_query($conn,$sql); while($r=mysqli_fetch_assoc($resultat)){ 
		 echo " <option value='".$r['id_m']."' >". $r['libelle']."</option>";}?></select></td>
<td><input type="text" name="pos2"></td>
	</tr>
	<tr>
		<td><select style="margin-bottom:10px;"  name="med3"><option value='' >Choisi...</option><?php  $resultat=mysqli_query($conn,$sql); while($r=mysqli_fetch_assoc($resultat)){ 
			echo " <option value='".$r['id_m']."' >". $r['libelle']."</option>";}?></select></td>
<td><input type="text" name="pos3"></td>
	</tr>
	<tr>
		<td><select style="margin-bottom:10px;"  name="med4"><option value='' >Choisi...</option><?php  $resultat=mysqli_query($conn,$sql); $resultat=mysqli_query($conn,$sql); while($r=mysqli_fetch_assoc($resultat)){ 
		 echo " <option value='".$r['id_m']."' >". $r['libelle']."</option>";}?></select></td>
<td><input type="text" name="pos4"></td>
	</tr>
	<tr>
		<td><select style="margin-bottom:10px;"  name="med5"><option value='' >Choisi...</option><?php  $resultat=mysqli_query($conn,$sql); while($r=mysqli_fetch_assoc($resultat)){ 
		 echo " <option value='".$r['id_m']."' >". $r['libelle']."</option>";}?></select></td>
<td><input type="text" name="pos5"></td>
	</tr>
	<tr>
		<td><select style="margin-bottom:10px;"  name="med6"><option value='' >Choisi...</option><?php  $resultat=mysqli_query($conn,$sql); while($r=mysqli_fetch_assoc($resultat)){ 
		 echo " <option value='".$r['id_m']."' >". $r['libelle']."</option>";}?></select></td>
	...
	<script src="table/js/main.js"></script>
		<script>
			...
			$(document).ready(function(){
				...
				//pour chercher les consultations par id 
				$(document).on ("click", "#chercher",function(){
					var code=$("#select").val();
					$.post("ordonnance/seggestiono.php", {
						cin:code
			 ...
			 // pour la suppression
			  $(document).on ("click", ".s", function () {
				...
				if (confirm("voulez-vous vraiment supprimer cette ordonnance?")) {
   				$(".don").load("ordonnance/deleteo.php",{
		...
		$("#phrase").html("modifier ordonnance");

       document.querySelector(".ajouter").style.visibility="visible";
            $(".ajouterf").load("ordonnance/editord.php",{
              id:this.parentNode.id
            })
    });
		  //fermer 
       ...
       document.querySelector(".ajouter").style.visibility="hidden";
            $(".ajouter").load("ordonnance/originalord.php",{
            })
    });
           ...
           if((d=="")||(f=="")){
            ...
            $(".don").load("ordonnance/searchord.php",{
            	debut:d,
            	fin:f
            });
        });
	  ...
	  var id=this.parentNode.id;
	  	...
	  	location.replace("infoord.php?id="+id);
			 ...
			 document.getElementById("fermer").addEventListener("click",function(){
			 					$("#phrase").html("ajouter ordonnance");
